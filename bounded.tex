...
\section{Intuition for Subeffecting}
Subtyping is used by many programming languages as a mean for making the type system for flexible. The normal intuition of subtyping is that if every value described by the type \li{S} is also described by \li{T}, then \li{S <: T}. The subtyping relation is also often interpreted as a subset relation, that is, if \li{S <: T}, then the elements of \li{S} are a subset of the elements of \li{T}. Type systems incorporates subtyping relation by adding the rule of subsumption:
$$\infer{\Gamma \vdash t : T}{\Gamma \vdash t : S & S <: T}$$
Which states that if a term \li{t} belongs to a type \li{S}, and \li{S} is a subtype if \li{T}, then \li{t} can be used as a value of type \li{T}.

There is a natural way to incorporate the interpretation of subtyping into the Wyvern effect system. Since the effect system describes the set of side effects a program can possibly cause, if one effect \li{E} is a subset of another effect \li{F}, then it is safe to use a term that has effect \li{E} whenever a term of effect \li{F} is expected. We therefore consider the effect \li{E} as a subeffect of the effect \li{F}, due to their similarity to subtyping. To give a concrete example, consider an declaration of a method \li{readFromNetwork}:\\
\begin{minipage}{\linewidth}
\begin{lstlisting}[xleftmargin=10pt, numbers=none]
def readFromNetwork() : {network.Read, file.Write} Unit = writeFile()
def writeFile() : {file.Write} Unit = file.Write{"something"}
\end{lstlisting}
\end{minipage}
The \li{readFromNetwork} method has an effect that consists of two effect labels, \li{network.Read} and \li{file.Write}, and is implemented by calling \li{writeFile} method, which only has the \li{file.Write} effect. Because the effect of \li{writeFile} method is a subset of the effect of \li{readFromNetwork}, it can be safely used as an implementation of \li{readFromNetwork}.
...
This main idea of the work of \citet{gordon13} is to control the access of user interface (UI) framework methods so that unsafe UI methods can only be called on the UI thread. There are three different method annotations \li{@SafeEffect}, \li{@UIEffect}, and \li{@PolyUIEffect}, where
\begin{enumerate}
...
\citet{grennhouse99} proposed a region-based effect system which describes how state may be accessed during the execution of some program component in object-oriented programming languages. One example of the usage of regions is as follows:
\begin{lstlisting}[numbers=none]
class Point {
...
Since Wyvern's effects are defined in terms of variables, before we typecheck expressions, we must make sure that effects and types are well formed. Wyvern well-formedness rules are mostly straightforward and are shown in Fig.~\ref{f-well-formedness}. The three judgements read that, in the variable typing context $\Gamma$, the type $\tau$, the declaration type $\sigma$, and the effect set $\varepsilon$ are well formed, respectively.
...
The S-Alg rule encodes the S-Refl-1, S-Perm, S-Depth, and S-Width rule using an injective function $p$. The subtyping rules of declaration types are identical to the declarative subtyping. We prove that S-Trans rules is admissible in theorem \ref{theorem-transitivity}. 

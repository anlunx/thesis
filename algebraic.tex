...
Consider the simple case where we only have two agents, namely the client \li{c}, and the host \li{h}. And the host \li{h} defines an abstract effect \li{E}, export a method that cause the effect and a method that handles the effect.
...
Now consider a client code wants to handle the \li{m} function from h\\
